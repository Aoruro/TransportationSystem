...
class LearningAStarSolver(AStarSolver):
    """
    Learning-enhanced A* Solver
    
    Integrates machine learning model predictions into A* search.
    
    f' = f - lambda * prob
    
    where prob is the model-predicted probability of reaching the goal.
    ...
    def __init__(self, dist_matrix: np.ndarray, model=None,
                 start: int = 0, lambda_param: float = 0.0):
        """
        Initialize learning-enhanced A* solver.
        
        Args:
            dist_matrix: NxN distance matrix
            model: Trained ML model for probability prediction
            start: Starting city index (default 0)
            lambda_param: Weighting parameter for ML guidance
        """
        super().__init__(dist_matrix, start)
        self.model = model
        self.lambda_param = lambda_param

    def _ml_heuristic(self, mask: int, current: int,
                     features: np.ndarray) -> Tuple[float, float]:
        """
        Compute ML-enhanced heuristic value.
        ...
        Returns:
            Tuple of (adjusted heuristic value, predicted probability)
        """
        base_h = self._heuristic(mask, current)

        if mask == self.full_mask or self.model is None:
            return base_h, 0.0
    ...
    def solve_with_features(self, feature_extractor) -> Dict:
        """
        Execute learning-enhanced A* search with feature extractor.
        
        Args:
            feature_extractor: Function to extract features from path
            
        Returns:
            Search result dictionary with path, cost, nodes_expanded, time, success
        """
        start_time = time.time()
        start_mask = 1 << self.start
        ...
        features = feature_extractor(initial_state.path, self.dist, len(self.dist))
        h0, _ = self._ml_heuristic(start_mask, self.start, features)
        ...
        while heap:
            ...
            for next_city in unvisited:
                ...
                if (new_mask, next_city) not in g_dict or new_g < g_dict[(new_mask, next_city)]:
                    g_dict[(new_mask, next_city)] = new_g
                    features = feature_extractor(new_path, self.dist, self.n)
                    h, prob = self._ml_heuristic(new_mask, next_city, features)
                    new_f = new_g + h
                    heapq.heappush(heap, (new_f, new_g, new_mask, next_city, new_path))
